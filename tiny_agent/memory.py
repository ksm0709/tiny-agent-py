...
class SessionMemory:
    ...
    def _spill_to_db(self, message: Dict[str, Any]):
        ...
        content_str = (
            json.dumps(content_val) if not isinstance(content_val, str) else content_val
        )
        short_content = (
    ...
    def list_archived_messages(self, limit: int = 50) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            rows = conn.execute(
                "SELECT id, timestamp, role, short_content FROM messages ORDER BY timestamp DESC LIMIT ?",
                (limit,),
            ).fetchall()
    ...
    def search_past_sessions(
        cls, query: str, limit: int = 20, db_dir: str = "~/.tiny-agent/sessions"
    ) -> List[Dict[str, Any]]:
    ...
    def cleanup_old_sessions(
        cls, days_old: int = 7, db_dir: str = "~/.tiny-agent/sessions"
    ):
        db_dir = os.path.expanduser(db_dir)
