...
class TestTinyAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_session_memory_spill_over(self):
        memory = SessionMemory(session_id="test_session", max_window_messages=2, db_dir=self.db_dir)
    ...
    async def test_agent_run(self, mock_acompletion):
        ...
        self.assertTrue(any(r.get("type") == "content" and r.get("content") == "Hello from mock!" for r in responses))
