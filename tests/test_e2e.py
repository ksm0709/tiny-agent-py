...
async def test_agent_end_to_end_flow(
    ...
    agent.hooks = {"pre_call": on_pre_call, "on_tool_start": on_tool_start}
...
@patch("tiny_agent.mcp_manager.MCPManager.connect_all")
@patch("tiny_agent.mcp_manager.MCPManager.close_all")
async def test_agent_reasoning_event(
    mock_close_all,
    mock_connect_all,
    mock_acompletion,
    mock_token_counter,
    ...
    responses = []
    async for chunk in agent.run("Think about it"):
        responses.append(chunk)

    reasoning_chunks = [r["content"] for r in responses if r.get("type") == "reasoning"]
    assert "Thinking..." in "".join(reasoning_chunks)
...
@patch("tiny_agent.mcp_manager.MCPManager.connect_all")
@patch("tiny_agent.mcp_manager.MCPManager.close_all")
async def test_agent_task_management_validation(
    mock_close_all,
    mock_connect_all,
    mock_acompletion,
    mock_token_counter,
    ...
    result = await agent.tools["manage_tasks"].execute(tasks=tasks)
    assert "Task list updated successfully" in result
    assert len(agent.tasks) == 2
    assert agent.tasks[0]["title"] == "Task 1"

    # Test turn_stop with pending tasks
    stop_result = await agent.tools["turn_stop"].execute(result="Done")
    assert "Error: Cannot stop turn" in stop_result
    assert "1 pending tasks remaining" in stop_result

    # Test turn_stop with no pending tasks
    agent.tasks[0]["status"] = "done"
    stop_result = await agent.tools["turn_stop"].execute(result="Done")
    assert "Turn stopped with result: Done" in stop_result
